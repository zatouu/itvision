    ...
    <div className="min-h-screen bg-stone-50 p-6">
      <div className="max-w-4xl mx-auto space-y-4">
        <div className="flex items-center justify-between">
          <Link href="/admin/achats-groupes" className="text-sm font-semibold text-stone-700 hover:underline inline-flex items-center gap-2">
        ...
        <div className="bg-white border rounded-2xl p-6 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-stone-900">Créer un achat groupé</h1>
              <p className="text-sm text-stone-600 mt-1">Sélectionne un produit, fixe les objectifs et la date limite.</p>
            </div>
            <button
              onClick={create}
              disabled={saving}
              className="inline-flex items-center gap-2 rounded-xl bg-stone-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60"
            >
          ...
          <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <label className="text-sm font-semibold text-stone-900">Produit</label>
                <div className="mt-2 flex gap-2">
                  ...
                  />
                  <div className="flex items-center text-xs text-stone-500">{loadingProducts ? '…' : `${products.length}`}</div>
                </div>
                ...
                {selectedProduct && (
                  <div className="mt-3 rounded-xl border bg-stone-50 p-3 text-sm">
                    <div className="font-semibold text-stone-900">{selectedProduct.name}</div>
                    ...
                    {Array.isArray(selectedProduct.priceTiers) && selectedProduct.priceTiers.length > 0 && (
                      <div className="mt-2">
                        <div className="text-xs font-semibold text-stone-700">Paliers de prix</div>
                        <div className="mt-1 grid grid-cols-2 gap-2">
                          {selectedProduct.priceTiers.slice(0, 6).map((t, idx) => (
                            <div key={idx} className="rounded-lg border bg-white p-2 text-xs">
                              <div className="text-stone-600">{t.minQty}+ unités</div>
                              <div className="font-bold text-stone-900">{formatCurrency(t.price, selectedProduct.currency || 'FCFA')}</div>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-semibold text-stone-900">Statut</label>
                  <select
                    className="mt-2 w-full rounded-xl border px-3 py-2 text-sm"
                    value={form.status}
                ...
                <div>
                  <label className="text-sm font-semibold text-stone-900">Deadline</label>
                  <input
                    type="date"
                    className="mt-2 w-full rounded-xl border px-3 py-2 text-sm"
                    value={form.deadline}
              ...
              <div>
                <label className="text-sm font-semibold text-stone-900">Transport</label>
                <select
                  className="mt-2 w-full rounded-xl border px-3 py-2 text-sm"
              ...
              <div>
                <label className="text-sm font-semibold text-stone-900">Description (optionnel)</label>
                <textarea
                  className="mt-2 w-full rounded-xl border px-3 py-2 text-sm"
                  rows={3}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-semibold text-stone-900">Objectif min</label>
                  <input
                    type="number"
                    className="mt-2 w-full rounded-xl border px-3 py-2 text-sm"
                    value={form.minQty}
                ...
                <div>
                  <label className="text-sm font-semibold text-stone-900">Objectif cible</label>
                  <input
                    type="number"
                    className="mt-2 w-full rounded-xl border px-3 py-2 text-sm"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-semibold text-stone-900">Max unités (optionnel)</label>
                  <input
                    type="number"
                    className="mt-2 w-full rounded-xl border px-3 py-2 text-sm"
                    value={form.maxQty ?? ''}
                ...
                <div>
                  <label className="text-sm font-semibold text-stone-900">Max participants (optionnel)</label>
                  <input
                    type="number"
                    className="mt-2 w-full rounded-xl border px-3 py-2 text-sm"
              ...
              <div className="rounded-2xl border p-4">
                <div className="text-sm font-bold text-stone-900">Contact affiché côté public</div>
                <div className="text-xs text-stone-600 mt-1">C’est le contact qui apparaît sur la page de l’achat groupé.</div>
              ...
              <div>
                <label className="text-sm font-semibold text-stone-900">Notes internes (optionnel)</label>
                <textarea
                  className="mt-2 w-full rounded-xl border px-3 py-2 text-sm"
                  rows={4}
