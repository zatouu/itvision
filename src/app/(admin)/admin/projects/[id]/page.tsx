...
const STATUS_COLORS: Record<string, string> = {
  lead: 'bg-stone-100 text-stone-700', quoted: 'bg-blue-100 text-blue-700',
  ...
  completed: 'bg-emerald-100 text-emerald-800', maintenance: 'bg-indigo-100 text-indigo-700',
...
const MSTATUS_COLORS: Record<string, string> = { pending: 'bg-stone-100 text-stone-600', in_progress: 'bg-blue-100 text-blue-700', completed: 'bg-emerald-100 text-emerald-800', delayed: 'bg-orange-100 text-orange-700' }
  ...
  if (!project) {
    return (
      <div className="p-6 text-center">
        ...
        <p className="text-stone-500">Projet introuvable</p>
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="bg-white border-b border-stone-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center gap-3 mb-3">
            <button onClick={() => router.push('/admin/projects')} className="p-2 hover:bg-stone-100 rounded-lg text-stone-500">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl font-bold text-stone-900 truncate">{project.name}</h1>
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${STATUS_COLORS[project.status] || 'bg-stone-100 text-stone-600'}`}>
              ...
              <p className="text-sm text-stone-500 mt-0.5 flex items-center gap-3 flex-wrap">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <SectionCard title="Client" icon={Building2}>
                  ...
                  ) : <p className="text-stone-500 text-sm">Aucune information client</p>}
                </SectionCard>
                ...
                {project.quote && (
                  <SectionCard title="Devis associé" icon={FileText}>
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-bold text-stone-900">{fmt(project.quote.totalTTC)}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm mt-2">
                      <span className="text-stone-600">Statut</span>
                ...
                <SectionCard title="Jalons" icon={Flag} action={{ label: 'Voir tout', onClick: () => setActiveTab('milestones') }}>
                  {(project.milestones || []).length === 0 ? (
                    <p className="text-sm text-stone-500">Aucun jalon défini</p>
                  ) : (
                    <div className="space-y-2">
                      {(project.milestones || []).slice(0, 5).map(m => (
                        <button
                          ...
                          className="w-full flex items-center justify-between text-sm p-2 rounded-lg bg-stone-50 hover:bg-blue-50 transition-colors text-left"
                        >
              ...
              <div className="space-y-6">
                <SectionCard title="Équipe" icon={User}>
                  {(project.assignedTo || []).length === 0 ? (
                    <p className="text-sm text-stone-500">Aucun technicien assigné</p>
                  ) : (
                    <div className="space-y-2">
          ...
          <div className="bg-white border border-stone-200 rounded-2xl p-6 space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <Field label="Progression (%)">
                ...
                <span className="text-sm text-stone-600">{editing.progress || 0}%</span>
              </Field>
              <Field label="Statut">
          ...
          <div className="space-y-4">
            <div className="bg-white border border-stone-200 rounded-2xl p-4">
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <label className="block text-sm font-medium text-stone-700 mb-1">Nouveau jalon</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">Échéance</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(editing.milestones || []).map(m => (
                <div key={m.id} className="bg-white border border-stone-200 rounded-xl p-4 flex items-start justify-between gap-3 group hover:shadow-sm transition-shadow cursor-pointer" onClick={() => setSelectedMilestone(m)}>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-stone-900 text-sm">{m.name}</p>
                    {m.dueDate && <p className="text-xs text-stone-500 mt-0.5">Échéance : {fd(m.dueDate)}</p>}
                  </div>
                  <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                    <select value={m.status} onChange={e => handleMilestoneStatus(m.id, e.target.value as any)} className="text-xs border border-stone-200 rounded-lg px-2 py-1">
              ...
              {(editing.milestones || []).length === 0 && (
                <div className="text-center py-10 text-stone-500 bg-white border border-stone-200 rounded-xl"><Flag className="w-10 h-10 mx-auto mb-2 opacity-30" /><p className="text-sm">Aucun jalon</p></div>
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === 'notes' && (
          <div className="space-y-4">
            <div className="bg-white border border-stone-200 rounded-2xl p-4">
              <label className="block text-sm font-medium text-stone-700 mb-2">Nouvelle note</label>
              ...
              {(project.sharedNotes || []).slice().reverse().map(note => (
                <div key={note.id} className="bg-white border border-stone-200 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold ${note.role === 'CLIENT' ? 'bg-emerald-100 text-emerald-800' : 'bg-violet-100 text-violet-700'}`}>{note.role === 'CLIENT' ? 'C' : 'A'}</div>
                    <div><p className="text-xs font-medium text-stone-900">{note.author}</p><p className="text-[10px] text-stone-400">{note.role} · {fd(note.createdAt)}</p></div>
                  </div>
                  <p className="text-sm text-stone-700 whitespace-pre-line">{note.message}</p>
                </div>
              ))}
              {(project.sharedNotes || []).length === 0 && (
                <div className="text-center py-10 text-stone-500"><MessageSquare className="w-10 h-10 mx-auto mb-2 opacity-30" /><p className="text-sm">Aucune note</p></div>
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === 'expenses' && (
          <div className="space-y-4">
            ...
            {loadingAssoc ? (
              ...
              <div className="text-center py-10 bg-white border border-stone-200 rounded-xl text-stone-500"><TrendingDown className="w-10 h-10 mx-auto mb-2 opacity-30" /><p className="text-sm">Aucune dépense liée</p></div>
            ) : (
              <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-stone-50 border-b border-stone-200"><tr><th className="text-left px-4 py-3 font-semibold text-stone-700">N°</th><th className="text-left px-4 py-3 font-semibold text-stone-700">Libellé</th><th className="text-left px-4 py-3 font-semibold text-stone-700">Date</th><th className="text-right px-4 py-3 font-semibold text-stone-700">Montant</th><th className="text-center px-4 py-3 font-semibold text-stone-700">Statut</th></tr></thead>
                  <tbody className="divide-y divide-stone-100">
                    {expenses.map(e => (
                      <tr key={e._id} className="hover:bg-stone-50">
                        <td className="px-4 py-3 font-mono text-xs text-stone-500">{e.numero}</td>
                        <td className="px-4 py-3 font-medium text-stone-900">{e.label}</td>
                        <td className="px-4 py-3 text-stone-600">{fd(e.expenseDate)}</td>
        ...
        {activeTab === 'documents' && (
          <div className="space-y-4">
            <div className="bg-white border border-stone-200 rounded-2xl p-4">
              <div className="flex items-end gap-3">
                <div className="flex-1"><label className="block text-sm font-medium text-stone-700 mb-1">Nom</label><input value={newDoc.name} onChange={e => setNewDoc({ ...newDoc, name: e.target.value })} className="input" placeholder="Nom du document" /></div>
                <div className="flex-1"><label className="block text-sm font-medium text-stone-700 mb-1">URL</label><input value={newDoc.url} onChange={e => setNewDoc({ ...newDoc, url: e.target.value })} className="input" placeholder="https://..." /></div>
                <div><label className="block text-sm font-medium text-stone-700 mb-1">Type</label><input value={newDoc.type} onChange={e => setNewDoc({ ...newDoc, type: e.target.value })} className="input" placeholder="pdf, xlsx..." /></div>
        ...
        {activeTab === 'photos' && (
          <div className="space-y-4">
            {loadingAssoc ? (
              ...
              <div className="text-center py-10 bg-white border border-stone-200 rounded-xl text-stone-500"><ImageIcon className="w-10 h-10 mx-auto mb-2 opacity-30" /><p className="text-sm">Aucune photo</p></div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {images.map(img => (
                  <div key={img._id} className="bg-white border border-stone-200 rounded-xl overflow-hidden group">
                    <a href={img.url} target="_blank" rel="noopener noreferrer" className="block aspect-square bg-stone-100 relative">
                      <img src={img.url} alt={img.title || img.filename} className="w-full h-full object-cover" />
                    </a>
                    <div className="p-3">
                      <p className="text-sm font-medium text-stone-900 truncate">{img.title || img.filename}</p>
                      {img.description && <p className="text-xs text-stone-500 truncate">{img.description}</p>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* TIMELINE */}
        {activeTab === 'timeline' && (
          <div className="space-y-0 relative pl-6 border-l-2 border-stone-200">
            {(project.timeline || []).length === 0 ? (
              <div className="text-center py-10 text-stone-500 bg-white border border-stone-200 rounded-xl"><Activity className="w-10 h-10 mx-auto mb-2 opacity-30" /><p className="text-sm">Aucun événement</p></div>
            ) : (
              (project.timeline || []).map((t, i) => (
                <div key={t.id || i} className="mb-6 relative">
                  ...
                  <div className="bg-white border border-stone-200 rounded-xl p-4 shadow-sm">
                    ...
                    <p className="text-[10px] text-stone-400 mt-2 flex items-center gap-1"><Calendar className="w-3 h-3" />{fd(t.date)}{t.author && <span>· {t.author}</span>}</p>
                  </div>
                </div>
              ))
            )}
        ...
        {activeTab === 'cashflow' && (
          <div className="space-y-6">
            ...
            <div className="bg-white border border-stone-200 rounded-2xl p-6">
              <h3 className="text-base font-semibold text-stone-900 mb-4">Répartition</h3>
              <div className="space-y-4">
...
function KpiCardMini({ label, value, color }: { label: string; value: string; color: string }) {
  ...
  return (
    <div className={`p-3 rounded-xl border ${map[color] || map.blue}`}>
      <p className="text-[11px] text-stone-500 font-medium">{label}</p>
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-sm mb-1">
        ...
        <span className="font-bold text-stone-900">{fmt(amount)}</span>
      </div>
