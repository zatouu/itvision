...
export default function PanierPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 pb-32 overflow-x-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-200 flex items-center gap-2">
              <ShoppingBag className="w-7 h-7 text-ddm-emerald" />
              Mon panier
              <span className="text-slate-500 dark:text-slate-400 text-lg font-normal">({items.length} article{items.length > 1 ? 's' : ''})</span>
            </h1>
          </div>
        ...
        {shippingUpgradeAvailable && (
          <div className="mb-6">
            <div className="bg-orange-50 dark:bg-orange-950/30 border-2 border-orange-200 dark:border-orange-800 rounded-xl p-4 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 h-2 bg-orange-100 dark:bg-orange-900/40 rounded-full overflow-hidden">
                  <motion.div
              ...
              <Link href="/produits" className="text-orange-700 dark:text-orange-400 font-medium text-sm whitespace-nowrap hover:underline">
                Ajouter →
              </Link>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-6">
          ...
          <div className="space-y-6">
            {/* Select all / delete */}
            <div className="flex items-center justify-between bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-3">
            ...
            <div className="border border-violet-200 dark:border-violet-900 rounded-xl overflow-hidden">
              <button
                onClick={() => setShowMaritimePanel(!showMaritimePanel)}
                className="w-full bg-violet-50 dark:bg-violet-950/30 p-3 flex items-center justify-between"
              >
                <span className="font-medium text-sm text-violet-900 dark:text-violet-300 flex items-center gap-2">
                  <Package className="w-4 h-4" /> Éligibilité fret maritime
                </span>
                {showMaritimePanel ? <ChevronUp className="w-4 h-4 text-violet-700 dark:text-violet-400" /> : <ChevronDown className="w-4 h-4 text-violet-700 dark:text-violet-400" />}
              </button>
              <AnimatePresence>
                {showMaritimePanel && (
                  <motion.div
              ...
              <section className="mt-6">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="hidden md:flex gap-2">
                    <button onClick={() => scrollSuggestions('left')} className="w-8 h-8 rounded-full border border-slate-200 dark:border-slate-700 flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-700">
                      <ChevronLeft className="w-4 h-4 text-slate-600 dark:text-slate-400" />
                    </button>
                    <button onClick={() => scrollSuggestions('right')} className="w-8 h-8 rounded-full border border-slate-200 dark:border-slate-700 flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-700">
            ...
            {activeGroups.length > 0 && (
              <section className="mt-6">
                <div className="bg-gradient-to-r from-violet-50 to-emerald-50 dark:from-violet-950/30 dark:to-emerald-950/30 border border-violet-200 dark:border-violet-900 rounded-2xl p-5">
                  <h3 className="font-bold text-lg text-slate-900 dark:text-slate-200 mb-1 flex items-center gap-2">
                    <Users className="w-5 h-5 text-violet-600 dark:text-violet-400" /> Économisez plus avec les achats groupés !
                  </h3>
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">Certains produits de votre panier ont des groupes actifs.</p>
            ...
            {breakdown.totalQuantity < 5 && (
              <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl p-4 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-amber-800 dark:text-amber-300 text-sm">Plus que {5 - breakdown.totalQuantity} produit(s) pour atteindre le minimum !</p>
                </div>
                <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">{breakdown.totalQuantity}/5</div>
              </div>
            )}
            {breakdown.totalQuantity >= 5 && breakdown.totalQuantity < 20 && (
              <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center flex-shrink-0">
                  <TrendingDown className="w-5 h-5 text-emerald-600" />
                </div>
                <div>
                  <p className="font-semibold text-emerald-800 dark:text-emerald-300 text-sm">🎉 Ajoutez {20 - breakdown.totalQuantity} produit(s) pour débloquer -5% supplémentaire !</p>
                </div>
              </div>
            )}
            ...
            {recentViewed.length > 0 && (
              <section className="mt-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-bold text-slate-900 dark:text-slate-200">Vus récemment</h3>
                </div>
                <div className="overflow-hidden max-w-full">
                  <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                    {recentViewed.slice(0, 6).map((product, idx) => (
                      <div
                        ...
                        className="w-[130px] flex-shrink-0 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-2 cursor-pointer hover:shadow-md transition"
                      >
                        <div className="h-20 bg-slate-100 dark:bg-slate-700 rounded-lg mb-2 overflow-hidden">
                          {product.image ? <img src={product.image} alt={product.name} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-slate-200 dark:bg-slate-600" />}
                        </div>
                        <p className="text-xs font-medium text-slate-900 dark:text-slate-200 line-clamp-2">{product.name}</p>
                        <p className="text-xs font-bold text-ddm-emerald dark:text-emerald-400 mt-1">{formatCurrency(product.price)}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <aside className="lg:sticky lg:top-24 h-fit">
            <div className="bg-white dark:bg-slate-800 border-2 border-emerald-100 dark:border-emerald-900/40 rounded-2xl p-5 space-y-4 shadow-sm">
              ...
              <div className="space-y-2 pt-2">
                <motion.button
                  ...
                  className="w-full h-14 bg-ddm-emerald hover:bg-ddm-emerald-dark disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-emerald-200 dark:shadow-emerald-900/30 transition"
                >
                  {sending ? 'Traitement...' : <>
                ...
                <button
                  ...
                  className="w-full py-3 border-2 border-slate-300 dark:border-slate-600 hover:border-ddm-emerald rounded-xl font-medium text-sm text-slate-700 dark:text-slate-300 flex items-center justify-center gap-2 transition"
                >
      ...
      <AnimatePresence>
        {showMobileSummary && items.length > 0 && (
          <motion.div
            ...
            className="fixed bottom-16 left-0 right-0 z-40 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] md:hidden"
          >
            <div className="flex items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                <p className="text-xs text-slate-500 dark:text-slate-400">Total ({breakdown.totalQuantity} article{breakdown.totalQuantity > 1 ? 's' : ''})</p>
                <p className="text-xl font-bold text-ddm-emerald dark:text-emerald-400">{formatCurrency(breakdown.total)}</p>
                {breakdown.totalSavings > 0 && <p className="text-[10px] text-ddm-emerald dark:text-emerald-400">Économie {formatCurrency(breakdown.totalSavings)}</p>}
              </div>
              <button
                onClick={proceedToCheckout}
      ...
      <div className="bg-slate-100 dark:bg-slate-900 mt-12 py-6 px-4 border-t text-center text-sm text-slate-600 dark:text-slate-400">
