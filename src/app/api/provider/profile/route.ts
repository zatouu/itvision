...
function normalizeProvider(pp: any) {
  return {
    ...
    availabilityStatus: pp.availabilityStatus || 'Disponible',
    visible: pp.visible !== false,
    scoreXeuy: pp.scoreXeuy ?? computeScoreXeuy(pp, { providerStats: pp.providerStats, kycVerified: pp.kycVerified, referralBalance: 0 }, { average: 0 }),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const scoreXeuy = computeScoreXeuy(providerProfile, user, reviewAgg || { average: 0, count: 0 })

    return NextResponse.json({
      success: true,
      user: normalizeUser(user),
      provider: normalizeProvider({ ...providerProfile, scoreXeuy }),
      scoreXeuy,
      kyc: {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (providerPatch.availabilityStatus !== undefined) {
      providerProfile.availabilityStatus = providerPatch.availabilityStatus
      providerProfile.visible = providerPatch.availabilityStatus === 'Disponible' ? (providerPatch.visible ?? providerProfile.visible ?? true) : false
    }
    if (providerPatch.visible !== undefined && providerProfile.availabilityStatus === 'Disponible') {
      providerProfile.visible = providerPatch.visible
    }
    ...
    const saved = providerProfile.toObject()
    saved.scoreXeuy = computeScoreXeuy(saved, user, {})
    return NextResponse.json({
      success: true,
