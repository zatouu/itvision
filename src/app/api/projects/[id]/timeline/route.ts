...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await Project.findByIdAndUpdate(
      id,
      { $push: { timeline: event } as any },
      { new: true }
    )
    if (!updated) return NextResponse.json({ error: 'Projet non trouvé' }, { status: 404 })
    return NextResponse.json({ success: true, project: updated })
  } catch (e) {
    return NextResponse.json({ error: 'Erreur ajout timeline' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await Project.findByIdAndUpdate(
      id,
      { $pull: { timeline: { id: eventId } } as any },
      { new: true }
    )
    if (!updated) return NextResponse.json({ error: 'Projet non trouvé' }, { status: 404 })
    return NextResponse.json({ success: true, project: updated })
  } catch (e) {
    return NextResponse.json({ error: 'Erreur suppression timeline' }, { status: 500 })
  }
}
