...
export default function CheckoutStepper({ currentStep }: CheckoutStepperProps) {
  ...
  return (
    <div className="sticky top-0 z-40 h-[60px] bg-white/90 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800">
            ...
            return (
              <div key={step.key} className="flex items-center flex-1 last:flex-initial">
                <div className="flex flex-col items-center gap-1.5">
                  ...
                  <span className={`text-[10px] font-medium ${isActive ? 'text-ddm-emerald' : isCompleted ? 'text-slate-700' : 'text-slate-400'}`}>
                    {step.label}
                  </span>
                </div>
                {idx < steps.length - 1 && (
