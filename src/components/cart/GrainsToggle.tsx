  ...
  return (
    <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-xl p-3">
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-sm font-medium text-slate-900">Utiliser {usable.toLocaleString('fr-FR')} Grains</p>
          <p className="text-xs text-slate-500">Économisez {valueFcfa.toLocaleString('fr-FR')} FCFA</p>
        </div>
      </div>
      <button
        type="button"
        ...
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${enabled ? 'bg-ddm-emerald' : 'bg-slate-300'}`}
      >
