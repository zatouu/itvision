          ...
          {quickCategories.map((cat) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-center gap-1.5 group flex-shrink-0 snap-start min-w-[3rem] md:min-w-[3.5rem]"
                title={cat.label}
              >
                <div className="w-11 h-11 md:w-12 md:h-12 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center group-hover:border-emerald-500 group-hover:ring-2 ring-emerald-500/20 transition shadow-sm">
                  <Icon className={`w-5 h-5 ${cat.color}`} />
                </div>
                <span className="hidden md:block text-[10px] text-slate-600 dark:text-slate-400 text-center leading-tight max-w-[4.5rem] line-clamp-2 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                  {cat.label}
                </span>
              </button>
            )
