  ...
  if (!stats) {
    return (
      <div className="bg-white rounded-xl border border-stone-200 p-8 text-center">
        <p className="text-stone-500">Aucune donnée disponible</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h2 className="text-2xl font-bold text-stone-900">Analytics & visiteurs</h2>
          <p className="text-sm text-stone-500 mt-1 flex flex-wrap items-center gap-1">
            <span className="font-medium text-stone-700">Derniers {days} jours</span>
            <span className="mx-1 text-stone-300">•</span>
            <span className={growthColor}>{growthLabel}</span>
          </p>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <span className="text-xs text-stone-500">Segment :</span>
            {PAGE_TYPES.map((type) => (
              <button
                key={type}
                type="button"
        ...
        <div className="flex items-center gap-3">
          <div className="inline-flex items-center rounded-full border border-stone-200 bg-white px-1 py-1 shadow-sm">
          ...
          <button
            onClick={loadStats}
            disabled={refreshing}
            className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-600 hover:bg-stone-50 transition disabled:opacity-50"
      ...
      {/* Onglets principaux */}
      <div className="mt-2 border-b border-stone-200 flex items-center gap-2 overflow-x-auto pb-1">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            type="button"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl border border-stone-100 p-6 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition">
          <div className="flex items-center justify-between mb-2">
            <div className="p-2 bg-blue-100 rounded-lg">
              <Eye className="h-5 w-5 text-blue-600" />
            </div>
            {stats.growth !== 0 && (
              <div className={`flex items-center gap-1 text-sm ${stats.growth > 0 ? 'text-emerald-700' : 'text-red-600'}`}>
                {stats.growth > 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
                <span>{Math.abs(stats.growth).toFixed(1)}%</span>
              </div>
            )}
          </div>
          <div className="text-2xl font-bold text-stone-900">{stats.totalVisits.toLocaleString('fr-FR')}</div>
          <div className="text-sm text-stone-500">Total visites</div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-6 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition">
          ...
          <div className="text-2xl font-bold text-stone-900">{stats.uniqueVisitors.toLocaleString('fr-FR')}</div>
          <div className="text-sm text-stone-500">Visiteurs uniques</div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-6 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition">
          ...
          <div className="text-2xl font-bold text-stone-900">{stats.authenticatedVisits.toLocaleString('fr-FR')}</div>
          <div className="text-sm text-stone-500">Visites authentifiées</div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-6 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition">
          ...
          <div className="text-2xl font-bold text-stone-900">{stats.anonymousVisits.toLocaleString('fr-FR')}</div>
          <div className="text-sm text-stone-500">Visites anonymes</div>
        </div>
      </div>
      {/* Contenu par onglet */}
      {activeTab === 'overview' && (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-semibold text-stone-900">Pages les plus visitées</h3>
                  <p className="text-xs text-stone-500 mt-0.5">
                    Vue filtrée sur : <span className="font-medium text-stone-700">{getPageTypeLabel(pageTypeFilter)}</span>
                  </p>
                </div>
              </div>
                  ...
                  <div key={page.path} className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-medium text-stone-900 truncate">{page.pageName}</div>
                        <div className="text-xs text-stone-500 truncate">{page.path}</div>
                      </div>
                    </div>
                    <div className="text-sm font-semibold text-stone-700">{page.visits}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              <h3 className="text-lg font-semibold text-stone-900 mb-4">Visites par type</h3>
              <div className="space-y-3">
          ...
          {stats.visitsByDay.length > 0 && (
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              <h3 className="text-lg font-semibold text-stone-900 mb-4">Évolution des visites</h3>
                ...
                {stats.visitsByDay.map((day) => {
                  ...
                  return (
                    <div key={day.date} className="flex-1 flex flex-col items-center gap-2">
                      ...
                      <div className="text-xs text-stone-500 transform -rotate-45 origin-top-left whitespace-nowrap">
      ...
      {activeTab === 'acquisition' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Origines du trafic */}
          <div className="bg-white rounded-xl border border-stone-200 p-6">
            <h3 className="text-lg font-semibold text-stone-900 mb-4">Origines du trafic</h3>
            <div className="space-y-3">
                  ...
                  <div key={item.referrer}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex-1 min-w-0 mr-2">
                        <div className="text-sm font-medium text-stone-700 truncate" title={item.referrer}>
                          {item.referrer}
                        </div>
                      </div>
                      <span className="text-sm font-semibold text-stone-700">{item.count}</span>
          ...
          <div className="space-y-6">
            {/* Top IPs */}
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              <h3 className="text-lg font-semibold text-stone-900 mb-1">Top IPs</h3>
              <p className="text-xs text-stone-500 mb-4">Sources les plus actives (volume)</p>
              <div className="space-y-3">
                {(stats.topIPs || []).length === 0 ? (
                  <p className="text-sm text-stone-500">Aucune IP disponible</p>
                ) : (
                  (stats.topIPs || []).slice(0, 10).map((item) => (
                    <div key={item.ip} className="flex items-center justify-between">
                      <span className="text-sm font-mono text-stone-800 truncate max-w-[220px]" title={item.ip}>
                        {item.ip}
                      </span>
                      <span className="text-sm font-semibold text-stone-700">{item.count}</span>
                    </div>
              ...
              {(stats.topAnonymousIPs || []).length > 0 && (
                <div className="mt-4 pt-4 border-t border-stone-100">
                  <p className="text-xs text-stone-500 mb-2">Top IPs anonymes</p>
                  <div className="space-y-2">
                    {(stats.topAnonymousIPs || []).slice(0, 5).map((item) => (
                      <div key={item.ip} className="flex items-center justify-between">
                        <span className="text-xs font-mono text-stone-700 truncate max-w-[220px]" title={item.ip}>
                          {item.ip}
                        </span>
                        <span className="text-xs font-semibold text-stone-600">{item.count}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              <h3 className="text-lg font-semibold text-stone-900 mb-4">Répartition par espace</h3>
              <div className="space-y-3">
                {stats.visitsByPageType.map((item) => {
                  const total = stats.visitsByPageType.reduce((sum, i) => sum + i.count, 0)
                  const percentage = total > 0 ? (item.count / total) * 100 : 0
      ...
      {activeTab === 'tech' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Appareils */}
          <div className="bg-white rounded-xl border border-stone-200 p-6">
            <h3 className="text-lg font-semibold text-stone-900 mb-4">Appareils</h3>
            <div className="space-y-3">
                  ...
                  <div key={item.device}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-stone-700 capitalize">{item.device}</span>
                      </div>
                      <span className="text-sm font-semibold text-stone-700">{item.count}</span>
                    </div>
                    <div className="w-full bg-stone-200 rounded-full h-2">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              <h3 className="text-lg font-semibold text-stone-900 mb-4">Navigateurs</h3>
              <div className="space-y-3">
                {stats.visitsByBrowser.map((item) => {
                  ...
                  return (
                    <div key={item.browser}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium text-stone-700">{item.browser}</span>
                        <span className="text-sm font-semibold text-stone-700">{item.count}</span>
                      </div>
                      <div className="w-full bg-stone-200 rounded-full h-2">
                        <div
            ...
            <div className="bg-white rounded-xl border border-stone-200 p-6">
              <h3 className="text-lg font-semibold text-stone-900 mb-4">Systèmes d'exploitation</h3>
              <div className="space-y-3">
                {stats.visitsByOS.map((item) => {
                  ...
                  return (
                    <div key={item.os}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium text-stone-700">{item.os}</span>
                        <span className="text-sm font-semibold text-stone-700">{item.count}</span>
                      </div>
                      <div className="w-full bg-stone-200 rounded-full h-2">
                        <div
      ...
      {activeTab === 'activity' && searchedRecentVisits.length > 0 && (
        <div className="bg-white rounded-xl border border-stone-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-lg font-semibold text-stone-900">Dernières visites (détails)</h3>
              <p className="text-xs text-stone-500 mt-1">
                IP source, rôle, device, navigateur, OS, referrer et user-agent
                {pageTypeFilter !== 'all' && (
                  <>
                    <span className="mx-1 text-stone-300">•</span>
                    <span className="font-medium text-stone-700">Segment : {getPageTypeLabel(pageTypeFilter)}</span>
                  </>
                )}
              </p>
            </div>
          ...
          <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <input
              ...
              className="w-full md:max-w-md border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-200"
            />
              ...
              <tbody>
                {searchedRecentVisits.slice(0, 20).map((visit) => (
                  <tr key={visit.id} className="border-b last:border-0">
                    ...
                    <td
                      className="py-2 pr-4 font-mono text-stone-800 cursor-pointer hover:text-emerald-700"
                    ...
                    <td className="py-2 pr-4">
                      <div className="text-stone-800 truncate max-w-[180px]" title={visit.pageName}>
                        {visit.pageName}
                      </div>
                      <div className="text-stone-400 text-xs truncate max-w-[180px]" title={visit.path}>
                        {visit.path}
                      </div>
                    </td>
                    <td className="py-2 pr-4 text-stone-700 capitalize">{visit.pageType}</td>
                    <td className="py-2 pr-4 text-stone-700">{visit.userRole || 'Anonyme'}</td>
                    <td className="py-2 pr-4 text-stone-700">{visit.isAuthenticated ? 'Oui' : 'Non'}</td>
                    <td className="py-2 pr-4 text-stone-700 capitalize">{visit.deviceType || '-'}</td>
                    <td className="py-2 pr-4 text-stone-700">{visit.browser || '-'}</td>
                    <td className="py-2 pr-4 text-stone-700">{visit.os || '-'}</td>
                    <td className="py-2 pr-4 text-stone-700 truncate max-w-[180px]" title={visit.referrer || undefined}>
                      {visit.referrer || '-'}
                    </td>
                    <td className="py-2 pr-4 text-stone-700 truncate max-w-[260px] hidden lg:table-cell" title={visit.userAgent || undefined}>
                      {visit.userAgent || '-'}
                    </td>
                  </tr>
                ))}
