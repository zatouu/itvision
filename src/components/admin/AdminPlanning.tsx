...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  pending: { label: 'En attente', color: 'bg-stone-100 text-stone-600' },
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <button onClick={() => setView('list')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${view === 'list' ? 'bg-blue-600 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}>
            <List className="w-4 h-4" /> Liste
          </button>
          <button onClick={() => setView('calendar')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${view === 'calendar' ? 'bg-blue-600 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        {[
          { label: 'Total', value: stats.total, color: 'bg-stone-50' },
          { label: 'Non assignées', value: stats.unassigned, color: 'bg-orange-50' },
          { label: 'En attente', value: stats.pending, color: 'bg-stone-50' },
          { label: 'Planifiées', value: stats.scheduled, color: 'bg-blue-50' },
          { label: 'En cours', value: stats.inProgress, color: 'bg-yellow-50' },
          { label: 'Terminées', value: stats.completed, color: 'bg-emerald-50' },
        ].map(s => (
          <div key={s.label} className={`${s.color} rounded-lg border border-stone-100 p-2.5 text-center`}>
      ...
      {loading && <div className="text-sm text-stone-400 py-8 text-center">Chargement...</div>}

      {/* Vue Liste */}
      {!loading && view === 'list' && (
        <div className="space-y-2">
          {data.length === 0 && <div className="text-sm text-stone-400 py-8 text-center">Aucune intervention</div>}
          {data.map(item => {
            const status = STATUS_LABELS[item.status] || { label: item.status, color: 'bg-stone-100' }
            return (
              <div key={item._id} className={`rounded-lg border ${PRIORITY_COLORS[item.priority] || 'border-stone-200'} bg-white p-3 flex items-start justify-between gap-3`}>
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-medium text-sm text-stone-900 truncate">{item.title}</span>
                  ...
                  <div className="flex items-center gap-3 mt-1 text-xs text-stone-400 flex-wrap">
                    ...
                    {item.contract && <span className="flex items-center gap-1 text-emerald-700"><Shield className="w-3 h-3" /> {item.contract.contractNumber}</span>}
                  </div>
                </div>
              </div>
            )
      ...
      {!loading && view === 'calendar' && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <button onClick={() => setCurrentMonth(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))} className="p-1 hover:bg-stone-100 rounded">
          ...
          <div className="grid grid-cols-7 gap-1">
            {['Dim', 'Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam'].map(d => (
              <div key={d} className="text-[10px] font-semibold text-stone-400 text-center py-1">{d}</div>
            ))}
            {Array.from({ length: firstDayOfWeek }).map((_, i) => (
              <div key={`empty-${i}`} className="h-20 rounded-lg bg-stone-50/50" />
            ))}
            {Array.from({ length: daysInMonth }).map((_, day) => {
              ...
              return (
                <div key={day} className="h-20 rounded-lg border border-stone-100 bg-white p-1 overflow-hidden">
                  <div className="text-[10px] font-medium text-stone-500 mb-0.5">{day + 1}</div>
                  <div className="space-y-0.5">
                    {dayItems.slice(0, 3).map(item => (
                      <div key={item._id} className={`text-[10px] truncate px-1 py-0.5 rounded ${item.status === 'completed' ? 'bg-emerald-50 text-emerald-800' : item.status === 'in_progress' ? 'bg-yellow-50 text-yellow-700' : 'bg-blue-50 text-blue-700'}`}>
                        {item.title}
                      </div>
                    ))}
                    {dayItems.length > 3 && (
                      <div className="text-[10px] text-stone-400 text-center">+{dayItems.length - 3}</div>
                    )}
                  </div>
                </div>
              )
