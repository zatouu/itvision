  ...
  const sendOtp = async () => {
    ...
    try {
      const r = await fetch(`${base}/api/auth/mobile/send-otp`, {
        ...
        body: JSON.stringify({ phone: cleaned, role: 'PROVIDER' }),
      })
    ...
    <SafeAreaView style={s.safe}>
      <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={s.top}>
          ...
          <Text style={s.title}>Xeuy Bi Pro</Text>
...
const s = StyleSheet.create({
  ...
  logo: { width: 72, height: 72, borderRadius: 20, backgroundColor: '#0F7B4F', alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
  ...
  btn: { backgroundColor: '#0F7B4F', borderRadius: 12, padding: 17, alignItems: 'center', marginTop: 8 },
