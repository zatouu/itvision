...
let RedisCtor = undefined
let redisLoadingPromise = null

async function loadIORedis() {
  if (RedisCtor !== undefined) return RedisCtor
  if (redisLoadingPromise) return redisLoadingPromise
  ...
  redisLoadingPromise = import('ioredis')
    .then((mod) => {
      // ioredis CJS exposes the Redis constructor as default and as Redis named export
      RedisCtor = mod.default || mod.Redis || mod
      return RedisCtor
    })
    .catch((err) => {
