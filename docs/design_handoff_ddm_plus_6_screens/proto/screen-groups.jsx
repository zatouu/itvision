// ============================================================
// SCREEN — LISTING ACHATS GROUPÉS
// ============================================================
const ScreenGroups = ({ device = "mobile" }) => {
      ...
      <Card className="overflow-hidden hover:shadow-md transition-shadow group cursor-pointer">
          ...
          <div className="absolute top-2 left-2">
            {g.status === "almost" ? (
              <span className="inline-flex items-center gap-1 whitespace-nowrap rounded-md bg-red-600 px-1.5 py-0.5 text-[9px] font-bold text-white">
                <Icon name="flame" size={9}/>Presque plein
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 whitespace-nowrap rounded-md bg-white/95 px-1.5 py-0.5 text-[9px] font-bold text-slate-900 dark:bg-slate-900/95 dark:text-white">
                <span className="h-1.5 w-1.5 rounded-full bg-red-500 animate-ping-slow"/>Live
              </span>
            )}
          </div>
        </div>
        <div className="p-2.5">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 truncate">{g.category}</p>
            <span className="inline-flex items-center gap-0.5 whitespace-nowrap text-[10px] font-bold text-red-600 dark:text-red-400 tabular-nums flex-shrink-0"><Icon name="clock" size={10}/>{g.deadline}</span>
          </div>
          <p className="mt-0.5 text-[12.5px] font-extrabold text-slate-900 dark:text-white line-clamp-2 leading-tight min-h-[32px]">{g.name}</p>

          <div className="mt-2">
            <ProgressBar value={g.currentQty} max={g.targetQty} tone="mixed" className="!h-1.5"/>
            <div className="mt-1 flex items-center justify-between text-[10px] font-semibold text-slate-500 dark:text-slate-400 tabular-nums">
          ...
          <div className="mt-2 flex items-baseline gap-1.5">
            <span className="text-[15px] font-extrabold text-violet-700 dark:text-violet-300 tabular-nums whitespace-nowrap">{fmtFCFA(g.unit)}</span>
            <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400">-{g.save}%</span>
          </div>
          <div className="mt-2">
            <Button variant="violet" size="sm" className="!h-8 !text-xs w-full">Rejoindre</Button>
          </div>
        </div>
      </Card>
    );
  ...
  // Simulateur MOQ palier
  const [simQty, setSimQty] = React.useState(15);
  const simTiers = PRODUCT.priceTiers;
  const simTier = simTiers.slice().reverse().find(t=>simQty>=t.from) || simTiers[0];
  const simSavings = (PRODUCT.basePrice - simTier.unit) * simQty;

  // MOBILE
  if (device === "mobile") {
    return (
      <div className="flex h-full flex-col bg-slate-50 dark:bg-slate-950">
        <MarketHeader back={()=>{}} title={null}/>
        <div className="flex-1 overflow-y-auto pb-20">
          ...
          <Section title="Simulateur — quel prix pour votre lot ?" subtitle="Ajustez la quantité pour voir le prix palier" className="mt-2 py-5 bg-white dark:bg-slate-900">
            <Card className="p-4">
              ...
              <input type="range" min={PRODUCT.minOrderQty} max={50} value={simQty} onChange={e=>setSimQty(+e.target.value)} className="w-full accent-emerald-600"/>
              <div className="mt-3 grid grid-cols-3 gap-2">
                <div className="rounded-xl bg-slate-50 p-2 dark:bg-slate-800">
                  ...
                  <p className="text-[15px] font-extrabold text-emerald-600 dark:text-emerald-400 tabular-nums">{fmtFCFA(simTier.unit)}</p>
                </div>
                <div className="rounded-xl bg-slate-50 p-2 dark:bg-slate-800">
                  <p className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">Total</p>
                  <p className="text-[15px] font-extrabold text-slate-900 dark:text-white tabular-nums">{fmtFCFA(simTier.unit*simQty)}</p>
                </div>
                <div className="rounded-xl bg-emerald-50 p-2 dark:bg-emerald-950/40">
                  ...
                  <p className="text-[15px] font-extrabold text-emerald-700 dark:text-emerald-300 tabular-nums">{fmtFCFA(simSavings)}</p>
                </div>
              </div>
        ...
        <MarketBottomNav device={device} active="groups"/>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-6xl px-6 py-6">
        ...
        <div className="mt-6 flex items-center justify-between gap-4">
          <div className="flex gap-2 overflow-x-auto">
            {cats.map((c)=>(
              <button key={c} onClick={()=>setCat(c)} className={cx("flex-shrink-0 rounded-full px-4 py-2 text-sm font-semibold whitespace-nowrap", cat===c?"bg-slate-900 text-white dark:bg-white dark:text-slate-900":"bg-white border border-slate-200 text-slate-700 hover:border-slate-300 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300 dark:hover:border-slate-600")}>{c}</button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-[1fr_auto] items-center gap-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-2xl border border-slate-200 p-3 dark:border-slate-800 w-32">
                <p className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">Prix / pc</p>
                <p className="mt-1 text-lg font-extrabold text-emerald-600 dark:text-emerald-400 tabular-nums">{fmtFCFA(simTier.unit)}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 p-3 dark:border-slate-800 w-32">
                <p className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">Total</p>
                <p className="mt-1 text-lg font-extrabold text-slate-900 dark:text-white tabular-nums">{fmtFCFA(simTier.unit*simQty)}</p>
              </div>
              <div className="rounded-2xl bg-emerald-50 p-3 dark:bg-emerald-950/40 w-32">
                ...
                <p className="mt-1 text-lg font-extrabold text-emerald-700 dark:text-emerald-300 tabular-nums">{fmtFCFA(simSavings)}</p>
              </div>
            </div>
          </div>
