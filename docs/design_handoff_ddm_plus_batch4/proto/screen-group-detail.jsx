// ============================================================
// SCREEN — DÉTAIL GROUPE
// ============================================================
const ScreenGroupDetail = ({ device = "mobile" }) => {
  const g = GROUPS[0]; // caméra 4MP
    ...
    <Card className="p-4">
      ...
      <div className="mt-3 rounded-xl bg-slate-50 p-3 dark:bg-slate-800">
        <div className="flex justify-between text-[12px]">
          ...
          <span className="font-bold text-violet-700 dark:text-violet-300 tabular-nums">{fmtFCFA(g.unit)}/pc</span>
        </div>
        <div className="mt-1 flex justify-between text-[12px]">
          <span className="text-slate-600 dark:text-slate-400">Sous-total</span>
          <span className="font-bold text-slate-900 dark:text-white tabular-nums">{fmtFCFA(g.unit * qty)}</span>
        </div>
        <div className="mt-1 flex justify-between text-[12px]">
          <span className="text-emerald-700 dark:text-emerald-300">Économie vs. seul</span>
          <span className="font-bold text-emerald-700 dark:text-emerald-300 tabular-nums">-{fmtFCFA((g.base - g.unit) * qty)}</span>
        </div>
      </div>
  ...
  const ShareCard = () => (
    <Card className="p-4">
      ...
      <div className="mt-3 flex items-center gap-1 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-1 pl-3 dark:border-slate-700 dark:bg-slate-800">
        ...
        <button onClick={copy} className={cx("flex items-center gap-1 rounded-lg px-3 py-1.5 text-[11px] font-bold transition-colors", copied ? "bg-emerald-600 text-white" : "bg-slate-900 text-white dark:bg-white dark:text-slate-900")}>
          {copied ? <React.Fragment><Icon name="check" size={12}/>Copié</React.Fragment> : <React.Fragment><Icon name="copy" size={12}/>Copier</React.Fragment>}
        </button>
      </div>
  ...
  const ParticipantsList = () => (
    <Card className="p-4">
      <div className="flex items-baseline justify-between">
        ...
        <a className="text-[12px] font-semibold text-slate-600 dark:text-slate-400">Tous</a>
      </div>
      ...
      <div className="max-h-72 space-y-3 overflow-y-auto p-3 bg-slate-50 dark:bg-slate-950">
        {messages.map((m,i)=>(
          <div key={i} className="flex items-start gap-2">
            <span className={cx("grid h-7 w-7 flex-shrink-0 place-items-center rounded-full text-[10px] font-extrabold text-white", m.staff ? "bg-emerald-600" : "bg-gradient-to-br from-violet-500 to-emerald-500")}>{m.i}</span>
            <div className={cx("min-w-0 flex-1 rounded-2xl px-3 py-2", m.staff ? "bg-emerald-50 border border-emerald-200 dark:bg-emerald-950/40 dark:border-emerald-900" : "bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800")}>
  ...
  if (device === "mobile") {
    return (
      <div className="flex h-full flex-col bg-slate-50 dark:bg-slate-950">
        <MarketHeader back={()=>{}} title={null} right={
          <React.Fragment>
            <button onClick={copy} className="grid h-9 w-9 place-items-center rounded-lg text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"><Icon name="share" size={18}/></button>
          </React.Fragment>
        }/>
          ...
          <div className="bg-white dark:bg-slate-900">
            ...
            <div className="p-4">
              ...
              <div className="mt-3 flex items-baseline gap-2">
                <span className="text-[28px] font-extrabold text-violet-700 dark:text-violet-300 tabular-nums">{fmtFCFA(g.unit)}</span>
                <span className="text-[13px] font-semibold text-slate-400 line-through tabular-nums">{fmtFCFA(g.base)}</span>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex items-baseline gap-1.5 whitespace-nowrap">
                <p className="text-[16px] font-extrabold text-violet-700 dark:text-violet-300 tabular-nums">{fmtFCFA(g.unit)}</p>
              ...
              <div className="grid grid-cols-2">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-4 flex items-baseline gap-2">
                    <span className="text-4xl font-extrabold text-violet-700 dark:text-violet-300 tabular-nums">{fmtFCFA(g.unit)}</span>
                    <span className="text-base font-semibold text-slate-400 line-through tabular-nums">{fmtFCFA(g.base)}</span>
                  </div>
